...
| `rsa(α,c)` | `RSASpeaker` | none | RSA S1 speaker. Ranks utterances by $\alpha \cdot \log L_0(t \mid u) - \text{cost} \cdot \text{len}$, takes argmax. |
...
| `rsa(α,c)` | `RSAListener` | none | L1. Inverts S1 via exact enumeration: $P(t \mid u) \propto S_1(u \mid t)$. |
| `vllm-listener(model,σ)` | `VLLMListener` | VLM | VLM predicts (x,y) coords; snapped to nearest object. Posterior via Gaussian kernel ($\sigma = 10$ default). |
...
| Brier Score | $\frac{1}{N}\sum (p_i - y_i)^2$ | Per-instance, lower is better |
| ECE | binned \|avg_conf − avg_acc\| | Calibration; 0 = perfect |
| H(T\|u) | $-\sum p_i \log p_i$ | Referential entropy; 0 = certain |
